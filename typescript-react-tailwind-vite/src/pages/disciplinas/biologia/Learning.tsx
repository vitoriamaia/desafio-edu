import LayoutSidebar from "@/widgets/LayoutSidebar/ui/LayoutSidebar/LayoutSidebar"; // Import LayoutSidebar
...
const Learning: FC = () => {
  ...
  const fetchResumo = async () => {
    ...
    try {
      const response = await fetch(LLAMA_API_URL, {
        ...
        body: JSON.stringify({
          model: LLAMA_MODEL,
          prompt: `Resuma o seguinte tema de biologia em um parágrafo simples: ${assunto}`,
          stream: false,
        }),
      });
  ...
  return (
    <section style={{ display: "flex", minHeight: "100vh" }}>
      {/* LayoutSidebar with similar functionality to Student */}
      <LayoutSidebar
      ...
      <div className="flex-1 p-6 bg-gray-100">
        <div className="max-w-3xl mx-auto rounded-lg shadow-lg overflow-hidden">
          <div className="bg-gradient-to-r from-blue-500 to-indigo-600 p-6">
            <h1 className="text-3xl font-semibold text-white text-center">{assunto}</h1>
          </div>
        </div>
