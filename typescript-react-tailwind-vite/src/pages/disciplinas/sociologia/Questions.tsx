      ...
      const requests = Array.from({ length: 2 }, async () => {
        try {
          const response = await fetch(LLAMA_API_URL, {
            ...
            body: JSON.stringify({
              model: LLAMA_MODEL,
              prompt: `Crie uma pergunta sobre o seguinte tema de sociologia: "${assunto}". O formato da resposta deve ser JSON: {"question": "Pergunta?", "options": ["A", "B", "C", "D"], "correctAnswer": "A"}.`,
              stream: false,
            }),
          });
